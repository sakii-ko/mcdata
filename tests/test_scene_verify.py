...
def _snow_profile_with_scene() -> dict:
    ...
    declared_scene = profile["world_state"]["scene"]
    runtime_scene = scene_mapping(load_scene(ROOT / "configs"))
    runtime_scene.update(declared_scene)
    runtime_scene["entries"] = scene_mapping(load_scene(ROOT / "configs"))["entries"]
    profile["world_state"]["scene"] = runtime_scene
    return profile


def _mutation_receipts(*, scene_count: int, biome_count: int) -> str:
...
def test_snow_profile_gate_accepts_all_scene_and_biome_receipts(tmp_path: Path) -> None:
    ...
    log.write_text(_mutation_receipts(scene_count=138, biome_count=5), encoding="utf-8")

    assert expected_count == 126 + 12 + 5
    assert (
        verify_scene_commands(
            log,
...
def test_snow_profile_gate_waits_for_every_biome_receipt(tmp_path: Path) -> None:
    log = tmp_path / "server.log"
    expected_count = expected_scene_fill_count(_snow_profile_with_scene())
    log.write_text(_mutation_receipts(scene_count=138, biome_count=4), encoding="utf-8")

    with pytest.raises(TimeoutError, match="142/143"):
        verify_scene_commands(
            log,
            expected_fill_count=expected_count,
            wait_sec=0.01,
...
def test_snow_profile_gate_waits_for_surface_overlay_receipts(tmp_path: Path) -> None:
    log = tmp_path / "server.log"
    expected_count = expected_scene_fill_count(_snow_profile_with_scene())
    log.write_text(_mutation_receipts(scene_count=126, biome_count=5), encoding="utf-8")

    with pytest.raises(TimeoutError, match="131/143"):
        verify_scene_commands(
            log,
            expected_fill_count=expected_count,
            wait_sec=0.01,
...
def test_scene_additions_require_variant_exact_origin_and_bounded_fill() -> None:
    addition = {
        "kind": "fill",
    ...
    with pytest.raises(ValueError, match="variant"):
        _scene_commands({"origin": [0, 64, 0], "entries": [], "additions": [addition]})
    ...
    with pytest.raises(ValueError, match="volume 40401 exceeds 32768"):
        _scene_commands(
            {
                "variant": "snow",
                "origin": [0, 64, 0],
    ...
    with pytest.raises(ValueError, match="replace filter is only valid for fill"):
        _scene_commands(
            {
                "variant": "snow",
                "origin": [0, 64, 0],
