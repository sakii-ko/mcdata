...
def test_run_matrix_uses_lane_trajectory_and_overrides(tmp_path: Path, monkeypatch) -> None:
    ...
    def fake_run_matrix_profiles(_root: Path, names: list[str], **kwargs) -> None:
        calls.append(("matrix", {"profiles": names, **kwargs}))

    monkeypatch.setattr(cli, "generate_strategy", fake_generate_strategy)
    monkeypatch.setattr(cli, "run_matrix_profiles", fake_run_matrix_profiles)
    ...
    matrix_calls = [call[1] for call in calls if call[0] == "matrix"]
    assert matrix_calls == [
        {
            "profiles": ["matrix_low", "matrix_textured"],
            "strategy": "ground_astar_loop",
            ...
            "replay_actions": False,
            "bootstrap": True,
            "trajectory_path": tmp_path / "runs" / "trajectories" / "ground_astar_loop_matrix_gpu1.json",
            "game_version": "26.2",
            "server_port": 25601,
            "lane": "gpu1",
            "probe_interval": 1.5,
...
def test_run_matrix_profiles_resolves_version_once_and_loops(tmp_path: Path, monkeypatch) -> None:
    calls: list[tuple[str, dict]] = []
    trajectory_path = tmp_path / "runs" / "trajectories" / "ground_astar_loop_matrix_main.json"
    trajectory_path.parent.mkdir(parents=True)
    trajectory_path.write_text('{"events": []}\n', encoding="utf-8")

    monkeypatch.setattr(pipeline, "load_profile", lambda _configs, _name: {"game_version": "26.2"})
    ...
    monkeypatch.setattr(pipeline, "bootstrap_profile", fake_bootstrap_profile)
    monkeypatch.setattr(pipeline, "launch_profile", fake_launch_profile)

    pipeline.run_matrix_profiles(
        tmp_path,
        ["matrix_low", "matrix_textured"],
        strategy="ground_astar_loop",
        duration=5,
        capture=False,
        with_server=False,
