...
| `mcdata.render` | config, paths, packs, resourcepacks, net, mojang, modrinth, manifest, runlog, settings, scene_model, action_curriculum/action_effect/action_source/reference_replay, action_combat/action_placement, actions.replay（输入回放，见注）, qa.probe（ffprobe 封装）, render.*（包内 lifecycle/scene/probe/placement/combat 分层） | actions 的策略实现（只消费 trajectory JSON 文件） |
