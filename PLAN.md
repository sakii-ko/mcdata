...
### T1h — 水池下沉（P10）+ toast 排查 + 窗口几何修复 + 终局重采

背景（planner 从 t1gl40sA 定位）：P9 修复生效后前 20s 偏差 ≤0.3 格；发散精确定位于 leg9（t≈21–24.6，z=12.5 向西），录像显示**水池漫流淹没平台**——水源建在 y=64 与步行面同层、无池沿，横向漫流 7 格覆盖 z∈[−9,14] 大片区域，水流减速+südwärts 推挤玩家（z 12.51→13.02 南漂实证）。该缺陷自 ITER-01 存在（P10）。另发现：右上角出现 toast 通知（违反录制要求）；l40s 上 `xwininfo -name Minecraft` 精确匹配失败（窗口实际标题为 "Minecraft* 26.2 - Multiplayer..."）导致回退全屏采集产生黑边——4090 无黑边只是窗口恰好占满全屏的巧合。**以下设计 planner 定死。**

1. **P10 水池下沉**（`server.py` 场景命令）：`blue_concrete` 层从 oy-1 移至 **oy-2**（y62），`water` 从 oy 移至 **oy-1**（y63）——水面低于步行面一格，四周由 y63 smooth_stone 天然围合，物理封闭且水反视效不变。blocked/route/golden 均无需变更（pool rect 本就在 blocked_rects）。场景镜像测试的占用格核对表同步（y64 层 pool 格恢复为空气，仍保持 blocked_rects 屏蔽）。
2. **toast 排查**：从 t1gl40sA 的 capture.mp4 抽 t=22–26 多帧，放大右上角识别 toast 内容与来源（嫌疑：narrator/教程/进度/配方解锁/探针命令反馈）；找到来源后用 options/gamerule/mod 配置消除（改动先在 report 里写明依据）；消除后的验证 run 抽帧确认右上角干净。
3. **窗口几何修复**（`pipeline._minecraft_window_geometry`）：改用 `xdotool search --name "Minecraft"`（子串匹配）取窗口列表，选面积最大者，`xdotool getwindowgeometry --shell` 解析位置尺寸；xdotool 不可用时保留现有 xwininfo 路径与全屏回退。同时 runlog 记录实测窗口尺寸；若窗口 ≠ 请求分辨率，记 warning（为何 l40s 窗口偏小待此数据再判）。QA 黑边检查预期在修复后转绿。
4. golden 不变；dev_check 全绿。
5. **验证 + 终局重采**（l40s-only，同 T1g 步骤 5）：连续 2×60s 双门 PASS（同时确认无黑边、无 toast）→ 四路重采 → 替换 `docs/qa_samples/iter02_4090_3way/`（目录名沿用）→ CephFS 归档 + 回传。停止条件照旧。

**验收**：toast 来源结论 + 消除证据；窗口几何 runlog 数据；验证与重采产物；dev_check 全绿。
