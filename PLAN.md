...
### T3 — Terrain × Action × Render 解耦扩展

1. 数据扩展遵循 `docs/data_diversity_contract.md`：pair 内锁定 terrain instance 与 action
   instance，只改变声明的 render edit axis；整体采用有确定性计划哈希的 sparse balanced
   crossing，不做不可控的全笛卡尔积。
2. 保持 `feedback_roam_10min` seed 401 向后兼容，新增 seeds 402–406 的五条同参数、两格净空、
   604 秒目标闭环路线；用 compact SHA-256 evidence 和语义测试代替五份超大 JSON golden。
3. 当前只有 seed 1 的单层 showcase plaza。首批新 terrain 先做河岸、森林步道、海岸、村庄、
   雪地和洞穴 gallery 的可验证单层安全 corridor；山地、自然洞穴、楼梯和浅水等任意高差路线
   在 height-aware `(x,y,z)` map/policy 完成前明确保持 blocked，不把二维 A* 冒充通用导航。
   Phase 2 已有未接 runtime 的 canonical `(x,feet_y,z)` surface/edge/A* 基础层，并以派生 artifact
   双 SHA 绑定当前 plaza；它不是新 terrain 的 snapshot/probe，也不解除上述 acceptance blocker。
4. release 数据必须报告 terrain/action/render degree、endpoint closure、pair invariant、route
   统计与 split leakage；held-out terrain/action/render family 按 group closure 整组隔离。

### T4 — 累积式 Action Curriculum 与可配比训练入口

1. action taxonomy 固定为四个累积 level：L1 `navigation`（真实移动与正常速率相机转动）、
   L2 `deliberate_jump`、L3 `deterministic_block_placement`、L4 `controlled_combat`。对应四个
   精确数据桶为 `l1`、`l1_l2`、`l1_l2_l3`、`l1_l2_l3_l4`；高等级 episode 必须同时包含并
   通过所有低等级能力，不把 L3/L4 当作可跳级的独立标签。
2. L2 的主动跳跃必须是轨迹显式计划且真实 dispatch 的 Space 输入，并由不慢于 0.20 秒最大
   采样间隔的 `positions.jsonl` 对四次动作逐一证明稳定 ground baseline、`peak_delta_y>=0.8`
   和落回 baseline；这份 `action_effect_report.json` 必须由 manifest 与 dataset index 双重 SHA
   绑定。feedback navigator 的
   stuck-recovery 跳跃单独统计，永远不能把 L1 样本升级为 L2。固定 pan 且没有真实位移不满足
   L1。planned capability、observed semantic counts、最高 observed level、控制器恢复次数和
   evidence artifact/hash 全部写入 run manifest。
3. L3 必须固定 hotbar 物品、放置目标坐标/方块/face，执行真实选槽与放置输入，并用服务端
   world-state probe 验证目标 block；L4 必须使用可重置、可控且身份固定的 combat encounter，
   执行真实攻击输入并验证目标实体/生命值或击杀状态。仅记录按键/鼠标 dispatch 不能单独证明
   L3/L4 成功；缺回执、目标漂移或 reset 失败均 fail closed，不进入 advanced bucket。
4. 每个 L3/L4 episode 开始前清理 inventory、掉落物和非玩家实体，恢复 action arena 的方块与
   entity snapshot；reset 命令数量和服务端回执进入 provenance。source/target render pair 共享
   完全相同的 terrain/action snapshot 和 action trajectory，结果性 world mutation 也必须按相同
   时刻、相同坐标/实体发生，不能让渲染差异改变 action 难度。
5. `dataset_index.json.action_buckets` 保存四桶精确、稳定排序的 episode ID/count。训练侧另生成
   hash-bound curriculum recipe：至少记录 dataset ID、stage、四桶采样比例、seed、replacement
   policy、group-closure/split hash 及展开后的 episode/pair ID；比例和为 1，引用空桶或跨 split
   泄漏必须拒绝。推荐依次训练 `l1` → `l1+l2` → `l1+l2+l3` → 全量，阶段间可显式保留简单桶
   replay 比例，禁止靠复制目录或 profile 文件名猜 level。
6. 每个 level 先通过同路线 60 秒真 GPU showcase、manifest/evidence 独立复算和视频目检，再
   扩到多 terrain/action seed；L1–L4 的 accepted 数量、失败原因、动作发生时刻与可视化标记都
   纳入 iteration report。尚未有真实 executor 的 level 必须保持空桶，不能用 contract-only
   事件冒充训练数据。

### T5 — External action sources 与 record-once/N-way replay

1. L1–L4 只表达“动作难度/效果是否被证明”；另设完全正交的 `action_source` taxonomy v1：
   `scripted_astar`、`feedback_planner`、`human_demo`、`learned_visual_policy`、
   `llm_skill_agent`。dataset index 保存逐 source 的稳定 episode ID/count，并提供显式 seed、
   exact-count、无放回采样入口；禁止从 profile 名或按键表面猜 source。
2. 建立 schema v1 canonical native action trace：固定 20 Hz，绑定 producer/version/model-or-config
   SHA、源 simulator/recorder 版本、**源 Minecraft 版本**、world seed/snapshot SHA；每 tick 保存
   held buttons、press/release edge、camera degree delta、hotbar/use/attack/inventory 和可选人工语义。
   内容 SHA 排除 self-hash 后对 canonical JSON 计算，同一输入转换必须 byte deterministic。
3. 第一阶段提供无重型依赖的 MineStudio/VPT spike：只消费经对应版本 mapper 解码后的
   MineStudio env-action dict；原始 hierarchical VPT token 不允许猜 mapping。另接 OpenAI VPT 7.x
   human recorder JSONL；GUI/未知 control、非 20 Hz、缺 checkpoint/模型 SHA 均 fail closed。
   degree→target-client mouse pixel 必须消费显式、SHA-bound calibration，禁止硬编码灵敏度。
4. 数据生成顺序固定为 **rollout 一次 → 保存 canonical trace → 每个 endpoint 恢复同一 snapshot
   → N-way replay**。严禁在每个材质/光影下独立运行 policy，否则观测域变化会污染 render edit。
   edit pair 的两个 endpoint 必须共享 `native_trace_sha256`；世界/实体/inventory/action effect 仍按
   T2/T4 门禁独立复算。
5. 人类 demo 归 `human_demo`；MineStudio/VPT 等视觉 policy 归 `learned_visual_policy`；
   Mineflayer/Voyager 风格的任务/技能 agent 归 `llm_skill_agent`，但其高层 task/code 只是 provenance，
   必须在执行边界采集 primitive trace，回放阶段不得再次调用外部 LLM。旧 scripted/feedback run
   只按已知 trajectory type + execution mode 机械迁移；未知 `external` 不得虚报成 A*。
6. compatibility gate 必须先证明 source snapshot 可在目标 MC 恢复、control 完整、20 Hz/timing、
   目标 client camera calibration、reference replay 的位置/yaw/inventory/entity/effect 与重复漂移。
   旧 MineRL/Minecraft 数据若不能通过 26.2 gate，只能作为 action prior/非严格 demo，不能进入
   exact prompt-edit pair。
7. T5 DoD：canonical schema + stable SHA、两个纯 adapter、确定性 trajectory compiler、golden/tests、
   manifest/dataset/pair source+trace 接线和 `docs/external_action_rollouts.md` 全部进入 dev_check；随后
   Phase 2 才安装/运行 simulator，完成一个 snapshot 的 neutral rollout、reference replay 和至少
   两个 render endpoint 的真 GPU compatibility smoke。未完成 semantic/effect binding 的原始
   jump/use/attack 只能标 `requires_semantic_effect_validation`，不得升级 L2–L4 bucket。
...
5. ~~外部 policy adapter（MineRL/VPT/Voyager）第一阶段~~ → 已移入 ITER-04 T5；重型 simulator
   rollout 与 target-version compatibility smoke 留在 T5 Phase 2。
6. ~~数据集打包器：扫描 runs 目录 → 汇总 episode 索引（manifest 聚合 + QA 通过标记）。~~ ✅ 2026-07-10 完成：fail-closed cohort/资源/QA/视觉验收、schema 与全文件 `SHA256SUMS`。
5b. ~~录制→轨迹转换契约~~ → ITER-04 T5 canonical 20 Hz trace；实时 recorder 仍属 Phase 2。
