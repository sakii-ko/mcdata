...
class NavigationConfig:
    ...
    def from_trajectory(cls, data: dict) -> NavigationConfig:
        ...
        config = cls(
            ...
            max_turn_px=int(raw.get("max_turn_px", 180)),
...
class _NavigationState:
    ...
    last_sequence: int
    first_pose: bool
    movement_samples: deque[tuple[float, float, float]]
...
class ServerLogPoseSource:
    ...
    def _poll(self) -> None:
        ...
        for raw_line in parts:
            ...
            elif len(values) == 2 and self._pending_position is not None:
...
def run_feedback_navigation(
    ...
    *,
    ...
    source = ServerLogPoseSource(server_log_path, username=username)
    try:
...
def _navigate(
    ...
    state = _NavigationState(1, 0, 0, -1, True, deque(), time.monotonic())
...
def _control_toward_waypoint(
    ...
    *,
    ...
    _log_control(
        ...
        moving=moving,
        mouse_dx=mouse_dx if turning else 0,
        config=config,
        state=state,
    )
    if moving and _is_stuck(state.movement_samples, config, now):
        _recover_or_raise(route_index, distance, config, state, controller, stop_event, nav_log)
...
def _turn_pixels(yaw_error: float, config: NavigationConfig) -> int:
    pixels = round(yaw_error * config.turn_px_per_degree)
