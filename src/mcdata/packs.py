...
def install_asset_set(
    ...
    *,
    ...
    resourcepack_sources: list[dict[str, Any]] = []
    ...
    for key in selected.get("resourcepacks", []) or []:
        ...
        if game_version not in version.game_versions:
            raise RuntimeError(
                f"Resource pack {key} version {version.version_number} does not declare "
        ...
        resourcepack_sources.append(
            {
                "asset_key": key,
                "project": str(spec["slug"]),
